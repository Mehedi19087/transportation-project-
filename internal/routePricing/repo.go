...
func (r *routePricingRepo) GetDealerNames() ([]string, error) {
    var dealerNames []string
    if err := r.db.Model(&RoutePricing{}).Distinct("dealer_name").Pluck("dealer_name", &dealerNames).Error; err != nil {
        return nil, err
    }
    return dealerNames, nil
}